...
api_router.get('/discos', function(req, res){
...
function queryDb(err, qry, callback){
	...
	sql.connect().query(qry, function(err, result){
		if(err){
			return error({code:[500], errorRes:["Ups! Algo ha fallado al intentar conectar con la BD."], errorLog:["Error (api_router>>discos): " + err]}, res);
		}
		callback.status(200);
